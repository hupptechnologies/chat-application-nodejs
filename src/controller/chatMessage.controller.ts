...
import { MessageStatus, SendMessageData, ChatHistoryData, UsersChatListData } from "../interface";
...
class ChatMessageController {
  ...
  private handleConnection(socket: Socket) {
    ...
    socket.on("message_delivered", async (data: { messageId: number; receiverId: number }) => {
      try {
        ...
        const updatedMessage = await ChatMessages.findOne({ where: { id: data.messageId } });
    ...
    socket.on("message_read", async (data: { messageId: number; senderId: number; receiverId: number }) => {
      try {
